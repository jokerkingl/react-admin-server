...
router.post('/manage/user/add', (req, res) => {
  // 读取请求参数数据
  const {username, password} = req.body
  // 处理: 判断用户是否已经存在, 如果存在, 返回提示错误的信息, 如果不存在, 保存 查询(根据username)
...
router.get('/manage/user/list', (req, res) => {
  UserModel.find({username: {'$ne': 'admin'}})
    .then(users => {http://localhost:3000/manage/category/info?categoryId=5e12b8bce31bb727e4b0e348
...
router.post("/manage/category/delete", (req, res) =>{
  ...
  CategoryModel.find({parentId: categoryId})
      .then(categories => {
        if(categories.length===0){
          ...
          res.send({status: 1, data: categories, msg: "该分类任有子分类"})
        }
      })
...
router.post("/manage/role/remove", (req, res) => {
  ...
  RoleModel.findOneAndDelete({_id: roleId})
      .then(()=>{
        res.send({status: 0})
      })
      .catch(error => {
